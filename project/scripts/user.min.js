...
window.gm.startDominoCombat = function (stopCB, startCB) {
    ...
    for (var i = data.teams.length - 1; i >= 0; i--) {
      for (var k = data.teams[i].length - 1; k >= 0; k--) {
        ...
        if (data.team === i || player.hp <= 0) {
          ...
          entry.addEventListener("click", function (target) {
            return selectTarget.bind(data, target);
          }(player.id));
        ...
        if (player.hp > 0 && player.id === data.player) {
          //list skills for curr. player
          ...
          for (var l = player.skills.length - 1; l >= 0; l--) {
            ...
            if (skill.cooldown > 0) {
              ...
              entry.addEventListener("click", function (player, skill) {
                return selectSkill.bind(data, player, skill);
              }(player.id, skill.id));
            }
  ...
  function newTurn() {
    ...
    for (var i = data.teams.length - 1; i >= 0; i--) {
      ...
      if (teamDead) {
        ...
        //start new round with team0 - note that all for-loops are operated backwards
        data.team = data.teams.length - 1;
        var x = data.teams[data.team].length - 1;
  ...
  function nextPlayer() {
    var player, _usethis; //skip to next alive in team
    ...
    for (var k = data.teams[data.team].length - 1; k >= 0; k--) {
      ...
      if (_usethis === true) {
        //the previous player was the current - this is next 
    ...
    if (_usethis === true) {
      //no more players in team, switch to next
      data.team -= 1;
  ...
  function selectSkill(playerId, skillId) {
    var playerA, playerB, player, skill, skillA;
